...
function generateAnimationSteps(searchValue) {
    const steps = [];
    const path = findSearchPath(searchValue);
    
    if (!path) {
        // Value not found - need to show the path taken
        let current = root;
        const searchPath = [];
        while (current) {
        ...
        for (let i = 0; i < searchPath.length - 1; i++) {
            ...
            steps.push({
                type: 'compare',
                message: `Step ${i + 2}: Compare ${searchValue} with ${current.value}. ${searchValue} ${searchValue < current.value ? '<' : '>'} ${current.value}, go ${direction}`,
        ...
        steps.push({
            type: 'notFound',
            message: `Step ${steps.length + 1}: Compare ${searchValue} with ${lastNode.value}. ${searchValue} ${searchValue < lastNode.value ? '<' : '>'} ${lastNode.value}, but ${searchValue < lastNode.value ? 'left' : 'right'} child is null. Value not found!`,
    ...
    steps.push({
        type: 'found',
        message: `Step ${steps.length + 1}: Compare ${searchValue} with ${path[path.length - 1].value}. Values match! Value found!`,
...
async function executeStep() {
    ...
    switch (step.type) {
        ...
        case 'found':
            highlightNode(step.currentNode, true);
            const foundNode = document.querySelector(`.node-${step.currentNode}`);
            if (foundNode) {
                const rect = foundNode.querySelector('rect');
                if (rect) {
...
function startVisualization() {
    ...
    animationSteps = generateAnimationSteps(searchValue);
    currentStepIndex = 0;
    isVisualizing = true;
    isPaused = false;
...
function reset() {
    ...
    document.getElementById('searchValue').value = '40';
    clearHighlights();
}
